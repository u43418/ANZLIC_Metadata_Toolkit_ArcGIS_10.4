...
<xsl:stylesheet version="1.0" exclude-result-prefixes="esri res t msxsl" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:t="http://www.esri.com/xslt/translator" xmlns:esri="http://www.esri.com/metadata/" xmlns:res="http://www.esri.com/metadata/res/" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
	...
	<xsl:template name="metadata">
		...
		<xsl:if test="(not(/metadata/mdFileID) or (/metadata/mdFileID = ''))">
			<xsl:message terminate="yes">Validation Fail: File Identifier</xsl:message>
		</xsl:if>		
		...
		<xsl:if test="(/metadata/mdChar/CharSetCd/@value = '' or not(/metadata/mdChar/CharSetCd))">
			<xsl:message terminate="yes"> Validation Fail: Character set</xsl:message>			
		</xsl:if>		
		...
		<xsl:if test="(/metadata/mdHrLv/ScopeCd/@value = 006 or /metadata/mdHrLv/ScopeCd/@value = 020 or /metadata/mdHrLv/ScopeCd/@value = 021 or /metadata/mdHrLv/ScopeCd/@value = 022 or /metadata/mdHrLv/ScopeCd/@value = 023 or /metadata/mdHrLv/ScopeCd/@value = 024)">
			<xsl:if test="(/metadata/mdParentID ='' or not(/metadata/mdParentID))">
				<xsl:message terminate="yes"> Validation Fail: Parent Identifier required</xsl:message>			
			</xsl:if>	
		</xsl:if>			
...
<!--ANZLIC: validate Hierarchy level-->		
		<xsl:if test="(/metadata/mdHrLv/ScopeCd/@value='005' and not(/metadata/mdHrLvName = 'dataset'))">
			<xsl:message terminate="yes">Validation Fail: Hierachy Name must be 'dataset'</xsl:message>
		</xsl:if>
		<xsl:if test="(not(/metadata/mdHrLv/ScopeCd/@value='005') and (/metadata/mdHrLvName = 'dataset'))">
			<xsl:message terminate="yes">Validation Fail: Hierachy Name can not be 'dataset'</xsl:message>
		</xsl:if>
<!--End: ANZLIC: validate Hierarchy level-->
		...
		<xsl:if test="not(/metadata/Esri/ArcGISProfile = 'NAP')">
			...
			<xsl:if test="(not(/metadata/mdHrLv/ScopeCd/@value='005') and (/metadata/mdHrLvName = ''))">
				<xsl:message terminate="yes">Validation Fail: Hierachy Name must be present and not be 'dataset'</xsl:message>
			</xsl:if>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:message terminate="yes">Validation Fail: either contact name, organisation or position must be present</xsl:message>
			</xsl:otherwise>						
		</xsl:choose>
		<xsl:choose>
...
<!--ANZLIC: validate dateStamp-->		
		<xsl:if test="((/metadata/mdDateSt[. = '']) or not(/metadata/mdDateSt))">
			<xsl:message terminate="yes">Validation Fail: Metadata Date Stamp required</xsl:message>
		</xsl:if>
<!--End: ANZLIC: validate dateStamp-->		
	...
	<xsl:template name="identificationInfo">
		...
		<xsl:if test="(not(idAbs) or idAbs ='')">
			<xsl:message terminate="yes">Validation Fail: Abstract required</xsl:message>
		</xsl:if>	
	...
	<xsl:template name="MD_DataIdentification">
		...
		<xsl:if test="(not(dataLang/languageCode) or dataLang/languageCode/@value='')">
			<xsl:message terminate="yes">Validation Fail: resource language required</xsl:message>
		</xsl:if>
		
		<!--ANZLIC: validate Dataset Languague character set-->
		<xsl:if test="(not(dataChar/CharSetCd) or dataChar/CharSetCd/@value='')">
			<xsl:message terminate="yes">Validation Fail: resource character set required</xsl:message>
		</xsl:if>
	...
	<xsl:template name="EX_Extent">	
		...
		<xsl:for-each select="geoEle/GeoBndBox[*]">
			<xsl:if test="(count(westBL) + count(eastBL) + count(southBL) + count(northBL)) != 4">
				<xsl:message terminate="yes">Validation Fail: Bounding box - west, east, south and north are required</xsl:message>
			</xsl:if>
...
<!-- ANZLIC Validation Responsable Party Role-->
		<xsl:if test="(not(role) or (role/RoleCd/@value = ''))">
			<xsl:message terminate="yes">Validation Fail: Resource Citation Contact Role is required'</xsl:message>
		</xsl:if>
<!-- ANZLIC Validation Responsable Party Role-->
	...
	<xsl:template name="CI_Citation">
		...
		<xsl:if test="(resTitle = '' or not(resTitle))">
			<xsl:message terminate="yes">Validation Fail: Citation Title is required'</xsl:message>
		</xsl:if>
		<!-- End: ANZLIC Validation Citation Title-->
		
		<!-- ANZLIC Validation Resource Reference Date-->
		<!-- The Create date was used-->
		<xsl:if test="date/createDate = ''">
			<xsl:message terminate="yes">Validation Fail: Citation Create Date is required'</xsl:message>
		</xsl:if>
		<!--xsl:if test="not(date/createDate)">
			<xsl:message terminate="yes">Validation Fail: Citation Create Date is required'</xsl:message>
		</xsl:if-->
	...
	<xsl:template name="CI_Date">
		<xsl:param name="type"/>
		<xsl:call-template name="dateOrDateTimeElements">
			<xsl:with-param name="eleName" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gts="http://www.isotc211.org/2005/gts" xmlns:srv="http://www.isotc211.org/2005/srv" xmlns:gml="http://www.opengis.net/gml" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">date</xsl:with-param>
		</xsl:call-template>
